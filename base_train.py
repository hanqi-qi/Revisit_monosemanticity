...
def train():
    ...
    if training_args.policy_paths is not None:
        ...
        policy_model = transformers.AutoModelForCausalLM.from_pretrained("/scratch/prj/lmrep/hanqi/attribute_edit/results/dpo_train/",device_map=device_map)
        tokenizer = transformers.AutoTokenizer.from_pretrained("/scratch/prj/lmrep/hanqi/attribute_edit/results/dpo_train/checkpoint-60")
    ...
    train_dataset = AlpacaSupervisedDataset(tokenizer=tokenizer, num_examples=99999, lorra_args=lorra_args,dataset_name=training_args.dataset_name)
