...
from models.model_utils import (
    prepare_position_ids_from_attention
)
...
def contrast_greedy_search(
    ...
    **model_kwargs,
    ...
    #for sanity check, the above input variables are all None
...
def forward_contrast_vector(
        ...
        #ToDo(Hanqi:check llama use sdpa attention, so only keep the second condition)
        ...
        if compute_contrast: #ToDo()
            ...
            #apply 2-d attention mask to learn the position_ids
            pos_position_ids = prepare_position_ids_from_attention(pos_attention_mask,past_key_values_length,position_ids=None)
            neg_position_ids = prepare_position_ids_from_attention(neg_attention_mask,past_key_values_length,position_ids=None)
        ...
        for layer_id, decoder_layer in enumerate(self.layers):
            ...
            hidden_states = layer_outputs[0] #[bs/1, seq_len, 4096]
            ...
            if compute_contrast:
                ...
                if layer_id in control_layer_ids: #
                    activations = alpha * (hidden_states_p[:, contrast_tokens:] - hidden_states_n[:, contrast_tokens:]) #[bs/1, |contrast_tokens|, 4096]
                    c_length = activations.shape[1] # |contrast_tokens|
