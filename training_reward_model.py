...
def compute_metrics(eval_pred):
    ...
    wandb.log({"rewards_j_mean": rewards_j_stats.mean[0]})
    wandb.log({"rewards_k_mean": rewards_k_stats.mean[0]})
    wandb.log({"rewards_total_mean": (rewards_j_stats.mean[0] + rewards_k_stats.mean[0]) / 2})
    wandb.log({"rewards_j_std": math.sqrt(rewards_j_stats.variance[0])})
    wandb.log({"rewards_k_std": math.sqrt(rewards_k_stats.variance[0])})
    wandb.log({"rewards_total_std": math.sqrt((rewards_j_stats.variance[0] + rewards_k_stats.variance[0]) / 2)})
...
def main(script_args):
    ...
    custom_eval_datasets = []
    # # Load custom eval set
    # if script_args.eval_dataset_name:
    #     cnt = 0
    #     for eval_set in script_args.eval_dataset_name:
    #         print(cnt, eval_set)
    #         custom_eval_dataset = load_dataset("json", data_files=eval_set, split="train")
    #         custom_eval_datasets.append(custom_eval_dataset)
    #         cnt+=1
    # else:
...
if __name__ == "__main__":
    ...
    #if script_args.report_to == 'wandb':
    wandb.init(project=script_args.wandb_project, group = script_args.wandb_run_name)
