...
class BasicTrainer(object):
    def __init__(
        ...
        self.dataset = AlpacaSupervisedDataset(tokenizer=self.tokenizer, num_examples=99999999999, lorra_args=config,dataset_name=config.dataset_name)
        
        self.train_iterator = self.dataset.get_pplm_batch_iterator(
            self.config,
            split="train",
        )
        
        self.eval_iterator = self.dataset.get_pplm_batch_iterator(
            self.tokenizer,
            self.config,
            split="valid",
        )
